pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/harishkemkar/java_project.git'
            }
        }
        ...
        stage('Build on Build Server') {
            steps {
                sshagent (credentials: ['build-server-key']) {
                    sh '''
                    ssh -o StrictHostKeyChecking=no ec2-user@${BUILD_SERVER_IP} \
                    "cd ${BUILD_DIR} && git pull && mvn clean package"
                    '''
                }
            }
        }
